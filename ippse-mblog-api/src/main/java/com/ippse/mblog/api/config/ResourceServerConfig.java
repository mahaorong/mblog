...
public class ResourceServerConfig extends ResourceServerConfigurerAdapter {
	...
	public void configure(HttpSecurity http) throws Exception {
		...
		/*http.authorizeRequests()
				.antMatchers("/actuator/**", "/favicon.ico", "/swagger**", "/", "/api/register/**", "/api/register",
						"/api/public/**", "/api/login/**")
		...
		http.cors().and().csrf().disable().authorizeRequests().antMatchers(HttpMethod.OPTIONS).permitAll()
				.antMatchers("/login**", "/webjars/**", "/error**", "/actuator/**", "/favicon.ico", "/swagger**", "/swagger/**", "/swagger**/**", "/v2/**").permitAll()
				.anyRequest().authenticated().and().oauth2Login().authorizationEndpoint().and().userInfoEndpoint()
				.customUserType(OkUser.class, "openkx").and().and().exceptionHandling()
